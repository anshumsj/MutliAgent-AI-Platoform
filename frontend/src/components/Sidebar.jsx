...
import { setConversations, setCurrentConversation, resetConversations } from "../redux/conversationSlice.js";
...
export default function Sidebar() {
  ...
  return (
    <aside
      ...
      } h-screen flex flex-col bg-[#0f0f14] border-r border-neutral-800/80 text-neutral-200 select-none shrink-0 transition-[width] duration-300 ease-in-out`}
    >
      {/* Top Header */}
      <div
      ...
      >
        {isCollapsed ? (
          <button
            ...
            className="text-neutral-400 hover:text-white transition-colors cursor-pointer p-1.5 rounded-lg hover:bg-neutral-800/60"
          >
          ...
          <>
            <div className="flex items-center gap-2.5">
              <button
                ...
                className="text-neutral-400 hover:text-white transition-colors cursor-pointer p-1 rounded-md hover:bg-neutral-800/60"
              >
                <LuPanelLeft className="w-5 h-5" />
              </button>
      ...
      <div className={isCollapsed ? "p-2 flex justify-center" : "p-3"}>
        {isCollapsed ? (
          <button
            ...
            disabled={creating}
            title="New Chat"
            className="w-10 h-10 flex items-center justify-center rounded-xl bg-[#161622] hover:bg-[#1f1e30] border border-neutral-800 text-neutral-300 hover:text-white transition-all cursor-pointer shadow-sm active:scale-95 disabled:opacity-50"
          >
      ...
      <div
        className={`flex-1 overflow-y-auto ${
          isCollapsed ? "px-2 space-y-2 flex flex-col items-center" : "px-3 space-y-1"
        } py-2 no-scrollbar`}
      >
        {!isCollapsed && (
                ...
                className={
                  isCollapsed
                    ? `w-10 h-10 flex items-center justify-center rounded-xl transition-all cursor-pointer ${
                        isSelected
                          ? "bg-purple-950/70 text-purple-300 border border-purple-800/60 shadow-sm shadow-purple-950"
                          : "text-neutral-400 hover:bg-neutral-800/50 hover:text-neutral-200"
                      }`
                    : `w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all cursor-pointer text-left truncate ${
                        isSelected
      ...
      {user && (
        <div
          className={`p-3 border-t border-neutral-800/60 flex ${
            isCollapsed ? "flex-col items-center gap-3" : "items-center justify-between"
          }`}
        >
          {isCollapsed ? (
            <>
              <div title={`${user.name} (${user.email})`} className="cursor-pointer">
                <UserAvatar avatar={user.avatar} name={user.name} size="sm" />
              </div>
              <button
                onClick={handleLogout}
                title="Sign out"
                className="text-neutral-400 hover:text-red-400 p-1.5 rounded-lg hover:bg-neutral-800/60 transition-colors cursor-pointer"
              >
